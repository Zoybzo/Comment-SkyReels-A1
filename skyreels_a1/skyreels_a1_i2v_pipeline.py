# Copyright 2024 The CogVideoX team, Tsinghua University & ZhipuAI and The
# HuggingFace Team.
# All rights reserved.
#
...
from \
    ...
    import _resize_with_antialiasing
...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        >>> prompt = "An astronaut hatching from an egg, on the surface of 
        the moon, the darkness and depth of space realised in the background. 
        High quality, ultrarealistic detail and breath-taking movie-like 
        camera shot."
        >>> image = load_image(
        ...     "https://huggingface.co/datasets/huggingface/documentation
        -images/resolve/main/diffusers/astronaut.jpg"
        ... )
...
# Similar to diffusers.pipelines.hunyuandit.pipeline_hunyuandit
...
def get_resize_crop_region_for_grid(src, tgt_width, tgt_height):
    ...
    return (crop_top, crop_left), (
        crop_top + resize_height, crop_left + resize_width)
...
def retrieve_timesteps(
    ...
    the scheduler after the call. Handles
    ...
    Args:
        ...
        num_inference_steps (`int`):
            ...
            pre-trained model. If used, `timesteps`
            must be `None`.
    ...
    Returns:
        `Tuple[torch.Tensor, int]`: A tuple where the first element is the
        timestep schedule from the scheduler and the
        second element is the number of inference steps.
    """
    if timesteps is not None and sigmas is not None:
        raise ValueError(
            "Only one of `timesteps` or `sigmas` can be passed. Please choose "
            "one to set custom values")
    if timesteps is not None:
        ...
        if not accepts_timesteps:
            raise ValueError(
                ...
                f"`set_timesteps` does not support custom"
                f" timestep schedules. Please check whether you are using the "
                f"correct scheduler."
            )
        ...
        if not accept_sigmas:
            raise ValueError(
                f"The current scheduler class {scheduler.__class__}'s "
                f"`set_timesteps` does not support custom"
                f" sigmas schedules. Please check whether you are using the "
                f"correct scheduler."
            )
...
# .pipeline_stable_diffusion_img2img.retrieve_latents
def retrieve_latents(
        encoder_output: torch.Tensor,
        generator: Optional[torch.Generator] = None, sample_mode: str = "sample"
):
...
class SkyReelsA1ImagePoseToVideoPipeline(DiffusionPipeline):
    ...
    Args:
        vae ([`AutoencoderKL`]):
            Variational Auto-Encoder (VAE) Model to encode and decode videos
            to and from latent representations.
        text_encoder ([`T5EncoderModel`]):
            Frozen text-encoder. CogVideoX uses
            [T5](https://huggingface.co/docs/transformers/model_doc/t5
            #transformers.T5EncoderModel); specifically the
            [t5-v1_1-xxl](https://huggingface.co/PixArt-alpha/PixArt-alpha
            /tree/main/t5-v1_1-xxl) variant.
        tokenizer (`T5Tokenizer`):
            Tokenizer of class
            [T5Tokenizer](https://huggingface.co/docs/transformers/model_doc
            /t5#transformers.T5Tokenizer).
        transformer ([`CogVideoXTransformer3DModel`]):
            A text conditioned `CogVideoXTransformer3DModel` to denoise the
            encoded video latents.
        scheduler ([`SchedulerMixin`]):
            A scheduler to be used in combination with `transformer` to
            denoise the encoded video latents.
    """
    ...
    def __init__(
            ...
            lmk_encoder: AutoencoderKLCogVideoX, # Pose Guider
            transformer: CogVideoXTransformer3DModel,
        ...
        Args:
            ...
            text_encoder:
            vae:
            lmk_encoder: Pose Guider; Perhaps the 3D Landmark Guider.
            transformer:
            scheduler:
            image_encoder:
            feature_extractor:
        ...
        self.vae_scale_factor_spatial = (
            2 ** (len(self.vae.config.block_out_channels) - 1) if hasattr(self,
                                                                          "vae") and self.vae is not None else 8
        )
        self.vae_scale_factor_temporal = (
            self.vae.config.temporal_compression_ratio if hasattr(self,
                                                                  "vae") and
                                                          self.vae is not
                                                          None else 4
        )
        self.vae_scaling_factor_image = (
            self.vae.config.scaling_factor if hasattr(self,
                                                      "vae") and self.vae is
                                              not None else 0.7
        )
    ...
    #         # We normalize the image before resizing to match with the
    ...
    #     # image_embeddings = image_embeddings.repeat(1,
    ...
    def _encode_image(self, image, device, num_videos_per_prompt,
                      do_classifier_free_guidance):
        """
        通过 siglip 对图片进行编码。SigLIP类似于CLIP，同样是一种多模态编码器。
        Args:
            image:
            device:
    ...
    # Copied from diffusers.pipelines.cogvideo.pipeline_cogvideox
    ...
    def encode_prompt(
        ...
        Args:
            ...
            negative_prompt (`str` or `List[str]`, *optional*):
                ...
                less than `1`).
            do_classifier_free_guidance (`bool`, *optional*, defaults to
            `True`):
                Whether to use classifier free guidance or not.
            num_videos_per_prompt (`int`, *optional*, defaults to 1):
                Number of videos that should be generated per prompt. torch
        ...
        if do_classifier_free_guidance and negative_prompt_embeds is None:
            ...
            if prompt is not None and type(prompt) is not type(negative_prompt):
                ...
                raise ValueError(
                    ...
                    f"{len(negative_prompt)}, but `prompt`:"
                    f" {prompt} has batch size {batch_size}. Please make sure "
                    f"that passed `negative_prompt` matches"
    ...
    def prepare_latents(
        ...
        """
        处理 latents 和 images，返回添加噪音后的 latents 和处理后的 image_latents
        Args:
            image: 图片/视频 embedding
            batch_size:
        ...
        if isinstance(generator, list): # TODO: 为什么需要一个list的generator？
            image_latents = [
        ...
        image_latents = torch.cat(image_latents, dim=0).to(dtype).permute(0, 2,
                                                                          1, 3,
                                                                          4) # [B, F, C, H, W]
        # [B, F, C, H, W]
    ...
    def prepare_control_latents(
        ...
        if mask is not None: # lmk_encoder是一个3D casual encoder，所以需要 mask
            mask = mask.to(device=device, dtype=self.lmk_encoder.dtype)
            bs = 1
            new_mask = []
            for i in range(0, mask.shape[0], bs):
                mask_bs = mask[i: i + bs] # 拿出来一个 batch 的 mask
        ...
        if masked_image is not None:
            masked_image = masked_image.to(device=device,
                                           dtype=self.lmk_encoder.dtype)
            bs = 1
            new_mask_pixel_values = []
            for i in range(0, masked_image.shape[0], bs):
                mask_pixel_values_bs = masked_image[i: i + bs]
                mask_pixel_values_bs = \
                    self.lmk_encoder.encode(mask_pixel_values_bs)[0]
                mask_pixel_values_bs = mask_pixel_values_bs.mode()
                new_mask_pixel_values.append(mask_pixel_values_bs)
            masked_image_latents = torch.cat(new_mask_pixel_values, dim=0)
            masked_image_latents = (masked_image_latents *
                                    self.lmk_encoder.config.scaling_factor)
        else:
    ...
    # .pipeline_animatediff_video2video.AnimateDiffVideoToVideoPipeline
    # .get_timesteps
    ...
    def check_inputs(
        ...
        if (
            ...
            raise ValueError(
                "`image` has to be of type `torch.Tensor` or "
                "`PIL.Image.Image` or `List[PIL.Image.Image]` but is"
                f" {type(image)}"
            )

        if height % 8 != 0 or width % 8 != 0:
            raise ValueError(
                f"`height` and `width` have to be divisible by 8 but are "
                f"{height} and {width}.")

        if callback_on_step_end_tensor_inputs is not None and not all(
                k in self._callback_tensor_inputs for k in
                callback_on_step_end_tensor_inputs
        ):
            raise ValueError(
    ...
    def unfuse_qkv_projections(self) -> None:
        ...
        if not self.fusing_transformer:
            logger.warning(
                "The Transformer was not initially fused for QKV projections. "
                "Doing nothing.")
        else:
    ...
    def __call__(
        ...
        Args:
            image (`PipelineImageInput`):
                The input image to condition the generation on. Must be an
                image, a list of images or a `torch.Tensor`.
            prompt (`str` or `List[str]`, *optional*):
                The prompt or prompts to guide the image generation. If not
                defined, one has to pass `prompt_embeds`.
                instead.
            negative_prompt (`str` or `List[str]`, *optional*):
                The prompt or prompts not to guide the image generation. If
                not defined, one has to pass
            ...
            self.transformer.config.sample_height *
            self.vae_scale_factor_spatial):
                The height in pixels of the generated image. This is set to
                480 by default for the best results.
            width (`int`, *optional*, defaults to
            self.transformer.config.sample_height *
            self.vae_scale_factor_spatial):
                The width in pixels of the generated image. This is set to
                720 by default for the best results.
            num_frames (`int`, defaults to `48`):
                Number of frames to generate. Must be divisible by
                self.vae_scale_factor_temporal. Generated video will
                contain 1 extra frame because CogVideoX is conditioned with (
                num_seconds * fps + 1) frames where
                num_seconds is 6 and fps is 4. However, since videos can be
                saved at any fps, the only condition that
            ...
            timesteps (`List[int]`, *optional*):
                Custom timesteps to use for the denoising process with
                schedulers which support a `timesteps` argument
                in their `set_timesteps` method. If not defined, the default
                behavior when `num_inference_steps` is
            ...
            *optional*):
            ...
            return_dict (`bool`, *optional*, defaults to `True`):
                ...
                .StableDiffusionXLPipelineOutput`] instead
                of a plain tuple.
            ...
            callback_on_step_end (`Callable`, *optional*):
                ...
                during the inference. The function is called
                with the following arguments: `callback_on_step_end(self:
                DiffusionPipeline, step: int, timestep: int,
                callback_kwargs: Dict)`. `callback_kwargs` will include a
                list of all tensors as specified by
                `callback_on_step_end_tensor_inputs`.
            callback_on_step_end_tensor_inputs (`List`, *optional*):
                The list of tensor inputs for the `callback_on_step_end`
                function. The tensors specified in the list
                will be passed as `callback_kwargs` argument. You will only
                be able to include variables listed in the
                `._callback_tensor_inputs` attribute of your pipeline class.
            max_sequence_length (`int`, defaults to `226`):
                Maximum sequence length in encoded prompt. Must be consistent
                with
                `self.transformer.config.max_text_seq_length` otherwise may
                lead to poor results.

        Examples:

        Returns:
            [`~pipelines.cogvideo.pipeline_output.CogVideoXPipelineOutput`]
            or `tuple`:
            [`~pipelines.cogvideo.pipeline_output.CogVideoXPipelineOutput`]
            if `return_dict` is True, otherwise a
            `tuple`. When returning a tuple, the first element is a list with
            the generated images.
        """

        if num_frames > 49:
            raise ValueError(
                "The number of frames must be less than 49 for now due to "
                "static positional embeddings. This will be updated in the "
                "future to remove this limitation."
            )
        ...
        # TODO: control_video 用途未知；可能是通过lmk_encoder提取3D Landmark
        ...
        control_video_latents_input = (
            ...
            do_classifier_free_guidance else control_video_latents
        )
        control_latents = rearrange(control_video_latents_input,
                                    "b c f h w -> b f c h w") # [B F C H W]

        # 6. Prepare extra step kwargs.
        # TODO: Logic should ideally just be moved out of the pipeline (原始TODO)
